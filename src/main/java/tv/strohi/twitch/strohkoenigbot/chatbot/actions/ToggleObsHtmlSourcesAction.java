...
public class ToggleBadgesSourcesAction extends ChatAction {
				...
				if (result) {
					if (enable) {
						if ("stroh_ohne_i".equals(args.getArguments().get(ArgumentKey.ChannelName))) {
							...
							args.getReplySender().send("Badges are now visible in stream. Don't forget to hide them again using \"!badges off\" as soon as you selected three!");
						}
					}
				}
			});
