...
public class DailyStatsSender implements ScheduledService {
	...
	public List<ScheduleRequest> createScheduleRequests() {
		return List.of(ScheduleRequest.builder()
			.name("DailyStatsSender_schedule")
			.schedule(CronSchedule.getScheduleString("0 15 * * * *"))
			.runnable(this::sendDailyStatsToDiscord)
			.build());
	}
...
//	@Scheduled(cron = "0 * * * * *")
	...
	private void sendDailyStatsAsPrivateMessage(Account account) {
		...
		if (!newBronzeBadgeWeapons.isEmpty()) {
			StringBuilder builder = new StringBuilder(message);
		...
		if (!newSilverBadgeWeapons.isEmpty()) {
			StringBuilder builder = new StringBuilder(message);
		...
		if (!newGoldBadgeWeapons.isEmpty()) {
			StringBuilder builder = new StringBuilder(message);
