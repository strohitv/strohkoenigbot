	...
	public void reloadBadges() {
		...
		for (Account account : accounts) {
			try {
				...
				var allBadgesYesterday = loadBadgesFailsafe(BADGES_FILE_PATH);
...
//				ImageIO.write(allBadgesImage, "png", outputfile);
					...
					saveBadgesFailsafe(BADGES_FILE_PATH, allOwnedBadges);
				}
			} catch (Exception e) {
	...
	private void saveBadgesFailsafe(String path, List<Badge> badges) {
	...
	private List<Badge> loadBadgesFailsafe(String path) {
		path = Paths.get(System.getProperty("user.dir"), path).toString();
		logger.debug("path '{}'", path);
